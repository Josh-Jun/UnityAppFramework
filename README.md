...
由于框架核心代码通过 Unity Package Manager 管理，导入第三方插件或新的 UPM 包时可能无法直接添加程序集引用。

接入流程：

1. 第三方功能在默认程序集或 `Builtin` 目录中创建桥接脚本
2. 将桥接脚本挂载到游戏对象上
3. 调用时找到对应GameObject对象
4. 使用GameObject.SendMessage("对应方法名的字符串"，参数或者参数数组)调用桥接脚本实现的方法
5. 需要回调时将回调函数当作参数传入即可
