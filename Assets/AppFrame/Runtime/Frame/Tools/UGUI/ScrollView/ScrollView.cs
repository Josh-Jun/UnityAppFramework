...
namespace UnityEngine.UI
{
    ...
    public class ScrollView : ScrollRect
    {
        ...
        public Action<int, RectTransform> initFunc;
        ...
        public virtual void SetInitFunc(Action<int,RectTransform> func)
        {
            initFunc = func;
        }
        ...
        {
            InternalScrollTo(m_dataCount - 1);
        }
        ...
        private void InternalUpdateData()
        {
            ...
            if (newDataCount != managedItems.Count)
            {
                if (managedItems.Count < newDataCount) //增加
                {
                    ...
                    {
                        ScrollItemWithRect itemWithRect = new ScrollItemWithRect();
                        RectTransform rt = GetNewItem(managedItems.Count);
                        rt.transform.SetParent(content, false);
                        initFunc?.Invoke(managedItems.Count, rt);
                        itemWithRect.item = rt;
                        itemWithRect.rect = rt.rect;
                        managedItems.Add(itemWithRect);
                    }
                }
                else //减少 保留空位 避免GC
                {
        ...
        Vector2 GetItemSize(int index)
        {
            ...
            if(index >= 0 && index <= m_dataCount)
            {
                ...
                if (managedItems[index].item != null)
                {
                    return managedItems[index].item.sizeDelta;
                }
            }
