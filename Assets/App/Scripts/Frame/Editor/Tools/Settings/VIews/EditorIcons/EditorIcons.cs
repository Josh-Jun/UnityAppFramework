...
{
    public class UnTools_EditorIcons : IToolkitEditor
    {
        private VisualElement btn_parent;
        private List<GUIContent> iconContentListAll;
        private List<GUIContent> iconContentListSmall;
        private List<GUIContent> iconContentListBig;
        private List<string> iconMissingNames;
        
        public void OnCreate(VisualElement root)
        {
            btn_parent = root.Q<VisualElement>("BtnParent");
            
            GetAllIcons();
            
            InitGUIContent();

            CreateIcons(iconContentListSmall, 30);
            
            root.Q<Button>("Small").clicked += () =>
            {
                CreateIcons(iconContentListSmall, 30);
            };
            
            root.Q<Button>("Big").clicked += () =>
            {
                CreateIcons(iconContentListBig, 60);
            };
        }

        private void GetAllIcons()
        {
            var all_icons = icon_list.Where(x => GetIcon(x) != null);
            ...
            icon_list = icon_list.ToList().Concat(unique).ToArray();
            Resources.UnloadUnusedAssets();
        }

        private void CreateIcons(List<GUIContent> list, int btn_size)
        {
            btn_parent.Clear();
            for (int i = 0; i < list.Count; i++)
            {
                GUIContent content = list[i];
                ...
                var iconWidth = content.image.width;
                var iconHeight = content.image.height;
                if (iconWidth > btn_size || iconHeight > btn_size)
                {
                    var ratio = content.image.width / content.image.height;
                
                    iconWidth = ratio > 0 ? btn_size : btn_size * content.image.width / content.image.height;
                    iconHeight =  ratio > 0 ? btn_size * content.image.height / content.image.width : btn_size;
                }
                ...
                VisualElement ve = new VisualElement()
                {
                    style =
                    {
                        width = iconWidth,
                        height = iconHeight,
                        backgroundImage = (Texture2D)content.image,
                    }
                };
                ...
                btn.RegisterCallback<MouseUpEvent>((evt) =>
                {
                    ...
                    Log.I($"已复制<color=#00ff00>[{content.tooltip}]</color>到剪切板！！！");
                });
                var s = $"Name: {content.tooltip}";
                s += $"\nSize: {content.image.width}x{content.image.height}";
                s += "\nIs Pro Skin Icon: " + (content.tooltip.IndexOf("d_") == 0 ? "Yes" : "No");
        ...
        private void InitGUIContent()
        {
            ...
            {
                GUIContent ico = GetIcon(icon_list[i]);

                if (ico == null)
                {
                    iconMissingNames.Add(icon_list[i]);
                    continue;
                }

                ico.tooltip = icon_list[i];
