...
namespace App.Runtime.CloudCtrl
{
    public static class CloudCtrlRequester
    {
        ...
        {
            var url = $"{Global.CdnServer}/App/{Application.identifier}/CloudCtrlConfig/{Global.PlatformName}/v{Application.version}/config.json?timestamp={DateTime.Now.Ticks}";
            UniTask.Void(async () =>
            {
                using var request = new UnityWebRequest();
                request.url = url;
